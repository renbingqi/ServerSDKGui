...
"deviceType": "Checkme_O2"} """)
        ...
        self.list_temp_label.setGeometry(10,215,600,50)
        #list ECG
        ...
        self.list_ecg_label.setGeometry(10,240,600,50)
        # list SpO2
        ...
        self.list_spo2_label.setGeometry(10, 265, 600, 50)
        # 日志栏
        self.log_textEdit=QTextEdit(self)
        self.log_textEdit.setGeometry(30,310,500,460)
        self.show()
    ...
    def add_device(self):
        ...
        try:
            ...
            if response.json()['message'] == 'success':
                self.log_textEdit.append(f'{eval(self.add_device_textEdit.toPlainText())["deviceName"]}添加成功')
            else:
                self.log_textEdit.append(f'{eval(self.add_device_textEdit.toPlainText())["deviceName"]}添加失败')
        except Exception as e:
            self.log_textEdit.append(e)
    def del_device(self):
        self.log_textEdit.clear()
        try:
            ...
            if response.json()['message'] == 'success':
                self.log_textEdit.append(f'{eval(self.add_device_textEdit.toPlainText())["deviceName"]}删除成功')
            else:
                self.log_textEdit.append(f'{eval(self.add_device_textEdit.toPlainText())["deviceName"]}删除失败')
        except Exception as e:
            self.log_textEdit.append(e)
    def show_router(self):
        ...
        for router in response.json()['data']:
            self.log_textEdit.append(str(router).replace(",",'\n').replace('{','').replace("}",'').replace(" ",''))
    ...
    def show_device(self):
        ...
        for device in response.json()['data']:
            self.log_textEdit.append(str(device).replace(",", '\n').replace('{', '').replace("}", '').replace(" ", ''))
            self.log_textEdit.append("----------------------------------------")
